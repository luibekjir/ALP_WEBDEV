                    ...
                    <div class="bg-white rounded-2xl shadow-md hover:shadow-xl transition duration-300 overflow-hidden flex flex-col h-full group">
                        ...
                        <div class="p-5 flex flex-col flex-grow">
                            ...
                            <div class="mt-auto flex items-center justify-between pt-3 border-t border-[#B8A5A8]/20">
                                <button class="flex items-center gap-1 text-[#5F1D2A]/80 hover:text-[#5F1D2A] transition">
                                    <svg xmlns="http://www.w3.org/2000/svg" class="h-5 w-5" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                                        <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M14 9l3 3m0 0l-3 3m3-3H7m6 0l-3-3m0 0l3-3" />
                                    </svg>
                                    Like ({{ $item->like_id ?? 0 }})
                                </button>
            ...
            <div class="bg-white rounded-lg shadow-lg w-11/12 md:w-1/2 lg:w-1/3 p-6 relative">
                ...
                <form action="{{ route('gallery.store') }}" method="POST" enctype="multipart/form-data">
                    ...
                    <div class="mb-4">
                        ...
                        <input type="text" name="name" id="name" class="mt-1 block w-full border border-[#B8A5A8] rounded-lg px-3 py-2 focus:outline-none focus:ring-2 focus:ring-[#5F1D2A]/50">
                    </div>
                    <div class="mb-4">
                        <label for="description" class="block text-sm font-medium text-[#5F1D2A]">Deskripsi</label>
                        <textarea name="description" id="description" rows="4" class="mt-1 block w-full border border-[#B8A5A8] rounded-lg px-3 py-2 focus:outline-none focus:ring-2 focus:ring-[#5F1D2A]/50"></textarea>
                    </div>
                    <div class="mb-4">
                        <label for="image" class="block text-sm font-medium text-[#5F1D2A]">Gambar</label>
                        <input type="file" name="image" id="image" class="mt-1 block w-full text-sm text-[#5F1D2A]/70 border border-[#B8A5A8] rounded-lg cursor-pointer focus:outline-none">
                    </div>
                    <div class="flex justify-end">
                        <button type="submit" class="bg-[#5F1D2A] text-white px-4 py-2 rounded-lg hover:bg-[#4a1620] transition">Simpan</button>
                    </div>
                </form>
            </div>
        </div>
