...
@section('content')
    ...
    <div class="w-full bg-[#FFF8F6] min-h-screen">
        <div class="container mx-auto py-12 px-6">
            <main class="w-full">
                ...
                @if ($events->isEmpty())
                    <div
                        class="flex flex-col items-center justify-center py-20 text-center bg-white border border-[#B8A5A8] border-dashed rounded-xl">
                ...
                @else
                    <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-2 gap-6">
                        @foreach ($events as $event)
                            <div
                                class="bg-white border border-[#B8A5A8] rounded-xl shadow-sm hover:shadow-md transition duration-300 overflow-hidden group flex flex-col h-full">
                                ...
                                <div class="p-4 flex flex-col flex-grow">
                                    ...
                                    {{-- Price + Button --}}
                                    <div
                                        class="mt-auto pt-3 flex items-center justify-between border-t border-[#B8A5A8]/20">
                                        ...
                                        <button
                                            class="bg-[#5F1D2A] text-white px-3 py-2 rounded-lg hover:bg-[#4a1620] transition text-sm font-medium">
                                            Daftar
                                        </button>
                                    </div>
                                </div>
    ...
    <div id="createEventModal" class="hidden fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50">
        <div class="bg-white rounded-lg shadow-lg w-11/12 md:w-1/2 lg:w-1/3 p-6 relative">
            ...
            <h2 class="text-2xl font-bold text-[#5F1D2A] mb-4">Tambah Acara Baru</h2>
    ...
    <script>
        ...
        const openEventModalBtn = document.getElementById('openEventModal'); // tombol untuk buka modal
